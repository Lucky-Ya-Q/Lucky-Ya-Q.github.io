...
public class JwtAuthenticationTokenFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
        ...
        String[] token = request.getHeader(header).split(" ");
        switch (token[0]) {
            case Constants.TOKEN_PREFIX:
                LoginUser loginUser = tokenService.getLoginUser(token[1]);
                ...
                break;
            case Constants.PORTAL_TOKEN_PREFIX:
                PortalLoginUser portalLoginUser = portalTokenService.getPortalLoginUser(token[1]);
                ...
                break;
            default:
                throw new ServiceException("token类型错误");
        }
...
1. 用户登录：http://127.0.0.1:8080/portal/login
...
2. 获取用户信息：http://127.0.0.1:8080/portal/getInfo
...
3. 退出登录（没登录也能调用）：http://127.0.0.1:8080/logout
